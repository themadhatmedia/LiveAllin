...
export class PlayerPage implements OnInit {
      ...
      switch (status) {
        ...
        default:
          ...
          if(this.isRepeat){
            ...
            var myindex = this.randomNumber(myindex,0, this.all.length);            
  ...
  randomNumber(myindex,min, max) {  
      min = Math.ceil(min); 
      max = Math.floor(max); 
      var index_randon =  Math.floor(Math.random() * (max - min + 1)) + min; 
      if(index_randon == myindex){
        this.randomNumber(myindex,min, max);
      }else{
        return index_randon;
      }
